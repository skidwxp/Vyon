...
function Library:Create(table)
    ...
    function tabHandler:Tab(name)
        ...
        main1.MouseButton1Click:Connect(
            function()
                for _, v in pairs(game.CoreGui:FindFirstChild("VyonLibrary").main:GetChildren()) do
                    if v.Name == "container" then
                        v.Visible = false
                    end
                end
                for _, v in pairs(game.CoreGui["VyonLibrary"].main.tabContainer:GetChildren()) do
            ...
            slider.MouseButton1Down:connect(
                function()
                    ...
                    while RunService.RenderStepped:wait() and down do
                        local percentage =
                            math.clamp((Mouse.X - frame1.AbsolutePosition.X) / (frame1.AbsoluteSize.X), 0, 1)
        ...
        function ElementHandler:Toggle(text, callback)
            ...
            toggle1.MouseButton1Click:Connect(
                function()
                    tog = not tog
                    callback(tog)
                    if tog then
                        game:GetService("TweenService"):Create(
                            frame3,
                            TweenInfo.new(0.2),
                            {BackgroundColor3 = Color3.fromRGB(83, 22, 149)}
                        ):Play()
                        game:GetService("TweenService"):Create(
                            uIStroke1,
                            TweenInfo.new(0.2),
                            {Color = Color3.fromRGB(83, 22, 149)}
                        ):Play()
                    else
                        game:GetService("TweenService"):Create(
                            frame3,
                ...
                function(Key)
                    if Key.UserInputType == Enum.UserInputType.MouseButton1 then
                        local ObjectPosition =
                            Vector2.new(Mouse.X - main.AbsolutePosition.X, Mouse.Y - main.AbsolutePosition.Y)
                        while RunService.RenderStepped:wait() and
                            UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                            local FrameX, FrameY =
                                math.clamp(Mouse.X - ObjectPosition.X, 0, dark_UI.AbsoluteSize.X - main.AbsoluteSize.X),
    ...
    game.CoreGui["VyonLibrary"].main.tabContainer.ChildAdded:Connect(
        function()
            game.CoreGui["VyonLibrary"].main:FindFirstChild("container").Visible = true
            pcall(
                function()
                    repeat
                        wait()
                    until game.CoreGui["VyonLibrary"].main:FindFirstChild("tabContainer"):FindFirstChildWhichIsA(
                        "TextButton"
                    )
                end
            )
            game:GetService("TweenService"):Create(
                game.CoreGui["VyonLibrary"].main:FindFirstChild("tabContainer"):FindFirstChildWhichIsA("TextButton"),
                TweenInfo.new(0.3),
                {TextTransparency = 0}
            ):Play()
        end
